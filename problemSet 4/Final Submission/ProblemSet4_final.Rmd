...
$$y_i=f(x_i)=\alpha+\beta_2\xi_1+\beta_3\xi^2+\beta_4\xi^3+\beta_5(\xi_1-\xi_1)^3+\epsilon_i \\ =\alpha+\beta_2\xi_1+\beta_3\xi^2+\beta_4\xi^3+\epsilon_i$$

Then take the 2nd derivative
...
$$f''(\xi_k)=2\beta_3+6\Big[\beta_4\xi_k+\beta_5(\xi_k-\xi_2)+...+\beta_{k+4}(\xi_k - \xi_{k-1})\Big]$$

$$\lim_{\epsilon \to 0}f''(\xi_k+\epsilon)=\lim_{\epsilon \to 0}f''(\xi_k-\epsilon)=f''(\xi_k) \\ =2\beta_3+6\left[\beta_4\xi_k+\beta_5(\xi_k-\xi_2)+...+\beta_{k+4}(\xi_k - \xi_{k-1})\right]$$
...
It turns a discrete optimization problem into one of continuous optimization. It selects $\lambda$, a tuning parameter using cross-validation, instead of number and location of knots. 
...
Like what we image, GAM is indeed the best model so far, combing the smoothing splines and predictors selected by LASSO.

Tree is a efficient and easy way to build a prediction model. A single tree is, however, a weak learner.The cost-complexity criteria tends to underfit trees. The penalty is for size is too high. The full tree, on the other hand, tends to overfits the data.

Thus, we need bootstrapping, random forest!!!
