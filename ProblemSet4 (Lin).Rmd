...
df_train.predictor <- as.matrix(df_train[,-19])
df_train.outcome <- as.matrix(df_train[,19])

df_test.predictor <- as.matrix(df_test[,-19])
df_test.outcome <- as.matrix(df_test[,19])
```
...
Lasso prediction
```{r}
fit.lasso <- glmnet(df_train.predictor, df_train.outcome, alpha = 1)
...
test.MSE <- data.frame(Models = c("Lasso", "GAM", "Full tree", "Pruned tree"),
                       MSE = c((mean(df_test$AcceptRate - prediction.lasso))^2,
                               (mean(df_test$AcceptRate - prediction.GAM))^2,
                               (mean(df_test$AcceptRate - prediction.tree))^2,
                               (mean(df_test$AcceptRate - prediction.ptree))^2))
