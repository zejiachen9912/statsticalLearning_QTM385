...
$\beta^* = \frac{\sum x_iy_i - \frac{1}{n}\sum x_i \sum y_i}{\beta^* (\sum (x_i)^2 - \frac{1}{n}(\sum x_i)^2}$
...
$$\frac{\partial J}{\partial \theta} =2(Y-X\theta)^t*(-X) = 2(-YX + \theta^tX^tX)$$
...
And the variance-covariance matrix will for $\hat{\beta} - cov[\hat{\beta}]$ will just be

$[(X'X)^{-1}X'y] - \sigma^2(X'X)^{-1} = (X'X)^{-1}(X'y - \sigma^2)$

Finally, we can have the standard erro by square rooting the diagonal of the matrix.
...
$Z * \frac{1}{\lambda}\int_{0}^{\infty} e^{-u} \,du = 1$

Then, we can solve for the integral,

$(-\frac{Z}{\lambda}) * e^{-u}\Big|_0^\infty = 1$

$(-\frac{Z}{\lambda}) * (e^{-\infty} - e^{-0}) = (-\frac{Z}{\lambda}) * (-1) = 1$
...
$E[f(x)] = \int_{0}^{\infty} x * \lambda * e^{-\lambda x} \,dx$
