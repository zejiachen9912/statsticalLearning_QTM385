...
print(paste("RMSE: ", knn_fit$results[3,2]))
...
print(paste("RMSE: ", round(sqrt(0.7677), 5)))
```
...
```{r}
gam1<-gam(shares~ s(global_subjectivity) + weekday_is_saturday + weekday_is_sunday
...
```{r}
RMSE <- c(round(sqrt(LOOCV), 5), knn_fit$results[3,2], round(sqrt(0.7676), 5), 0.89204, 
          round(sqrt(0.720559), 5),
          round(sqrt(boost_tree_2$cv.error[1000]), 5))
...
**GAM**
```{r}
prediction.GAM <- predict(gam1, newdata = tst_x_df)
```
...
```{r}
prediction.bRF <- predict(boost_tree_2, newdata = tst_x_df)
```

Compare the MSE for these predictions
```{r}
test.RMSE <- data.frame(Models = c("Linear","KNN","Lasso", "GAM", "Random Forest", "Boosted Rnadom Forest"),
                       RMSE = c(ModelMetrics::rmse(pred=prediction.linear, actual = df_test$shares),
                               ...
                               ModelMetrics::rmse(pred=prediction.GAM, actual = df_test$shares), 
                               ModelMetrics::rmse(pred=prediction.RF, actual = df_test$shares), 
                               ModelMetrics::rmse(pred=prediction.bRF, actual = df_test$shares)))
